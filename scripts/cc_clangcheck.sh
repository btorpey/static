...
which clang-check 2>&1 >/dev/null
if [[ $? -ne 0 ]]; then
   >&2 echo "clang-check not found!"
   exit 1
fi
...
TEMPFILE=$(mktemp /tmp/clangcheck-XXXX)
rm -f ${TEMPFILE} 2>&1 >/dev/null

# iterate over compilation db and generate results
${SCRIPT_DIR}/cc_driver.pl "${passThru[@]}" `which clang-check` -analyze >${TEMPFILE}
if [[ $? != 0 ]]; then
   echo "..."
   tail ${TEMPFILE}
